...
function firstQuestion() {
    $('.content').hide();
    var timer = null; // declare a timer variable
    Swal.fire({
        ...
        clearTimeout(timer);
        $('.content').show(200);
        var audio = new Audio('sound/Ligaya2.mp3');
        audio.volume = 0.1; 
        audio.play();
    });
    
    // set the timer to click the button after 20 seconds
...
$('#yes').click(function() {
    ...
    Swal.fire({
        ...
        width: 900,
        padding: '3em',
        html: "<input type='text' class='form-control' id='txtReason' onmousemove=textGenerate()  placeholder='Say Yes!'>",
        background: '#fff url("img/iput-bg.jpg")',
        backdrop: `
              rgba(0,0,123,0.4)
              url("img/giphy2.gif")
